...
export function AnimatedStat({ value, label }: { value: number; label: string }) {
  ...
  return <motion.div ref={ref} className={cn("rounded-card border border-cyan-500/30 bg-gradient-to-br from-cyan-500 to-blue-600 p-6 text-white shadow-md shadow-blue-600/15")} whileHover={reduceMotion ? undefined : { y: -4 }} transition={{ duration: 0.25 }}><b className="text-4xl font-extrabold text-white">{shown.toLocaleString("fa-IR")}</b><p className="mt-2 text-sm font-medium text-cyan-100">{label}</p></motion.div>;
}
