...
import { createSubAdmin, toggleAdminStatus, deleteSubAdmin } from "./actions";
import { UserPlus, Shield, UserCheck, Trash2, Power } from "lucide-react";
  ...
  return (
    <>
      ...
      <div className="mt-6 rounded-card bg-white p-6 shadow-sm border border-navy-900/10">
        ...
        <form action={createSubAdmin} className="grid gap-4 sm:grid-cols-3">
              ...
              return (
                <tr key={user.id} className="border-b border-navy-900/10 hover:bg-slate-50/50">
                  ...
                  <td className="p-4">
                    ...
                    {isSuper && <span className="text-xs text-ink-900/40">حساب اصلی سیستم</span>}
                  </td>
                </tr>
              );
            })}
